...
class SubProblem:
    def __init__(self, N: int, M: int):
        ...
        self.clusterBits = [{} for _ in range(N + 1)] # Bits for the mechanisms of A1,A2,..,An
        self.beta_varsX0 = {}
        self.beta_varsX1 = {}
    ...
    def setup(self, amountBitsPerCluster: int, amountBetaVarsPerX: int, duals: dict[int, float], amountNonTrivialRestrictions: int,
        ...
        for clusterIndex in range(1, N + 1):
            ...
            for i in range(amountBitsPerCluster): clusterBitsNames.append(f"cluster_{clusterIndex}_bit_{i}")

            self.clusterBits[clusterIndex] = self.model.addVars(amountBitsPerCluster, obj=0,
                                              vtype=GRB.BINARY, name=clusterBitsNames)
        ...
        self.bitsParametric = self.model.addVars(amountNonTrivialRestrictions, obj=[-duals[dualKey] for dualKey in duals],
                                                   vtype=GRB.BINARY, name=parametricColumnsNames)        
        
        # Constraints for beta VarX0:
        for betaVarX0Index in range(len(betaVarsBitsX0)):
            self.model.addConstr(self.beta_varsX0[betaVarX0Index] >= 0,
                                        name=f"BetaX0_{betaVarX0Index}")
            self.model.addConstr(self.beta_varsX0[betaVarX0Index] <= 1,
                                        name=f"BetaX0_{betaVarX0Index}")
            
            for bitPlus in betaVarsBitsX0[betaVarX0Index][0]:
                parts = bitPlus.split('_')
                clusterBitIndex = int(parts[0][1:]); bitIndex = int(parts[1])                
                self.model.addConstr(self.beta_varsX0[betaVarX0Index] <= self.clusterBits[clusterBitIndex][bitIndex],
                                        name=f"BetaX0_{betaVarX0Index}_BitPlus_{bitPlus}")

            for bitMinus in betaVarsBitsX0[betaVarX0Index][1]:
                parts = bitMinus.split('_')
                clusterBitIndex = int(parts[0][1:]); bitIndex = int(parts[1])
                self.model.addConstr(self.beta_varsX0[betaVarX0Index] <= 1 - self.clusterBits[clusterBitIndex][bitIndex],
                                        name=f"BetaX0_{betaVarX0Index}_BitMinus_{bitMinus}")

        self.constrs = self.model.addConstrs(( gp.quicksum(self.clusterBits[int(bitPlus.split('_')[0][1:])][int(bitPlus.split('_')[1])]
                                                for bitPlus in betaVarsBitsX0[indexBetaVarX0][0]) +
                                            gp.quicksum(1 - self.clusterBits[int(bitMinus.split('_')[0][1:])][int(bitMinus.split('_')[1])]
                                                for bitMinus in betaVarsBitsX0[indexBetaVarX0][1]) +
                                            1 - (len(betaVarsBitsX0[indexBetaVarX0][0]) + len(betaVarsBitsX0[indexBetaVarX0][1]))
                                        <= self.beta_varsX0[indexBetaVarX0] for indexBetaVarX0 in range(len(self.beta_varsX0))), 
                                        name="betaX0_Force1")
        
        # ------ Constraints for beta VarX1:
        for betaVarX1Index in range(len(betaVarsBitsX1)):
            self.model.addConstr(self.beta_varsX1[betaVarX1Index] >= 0,
                                        name=f"BetaX1_{betaVarX1Index}")
            self.model.addConstr(self.beta_varsX1[betaVarX1Index] <= 1,
                                        name=f"BetaX1_{betaVarX1Index}")
            
            for bitPlus in betaVarsBitsX1[betaVarX1Index][0]:
                parts = bitPlus.split('_')
                clusterBitIndex = int(parts[0][1:]); bitIndex = int(parts[1])
                self.model.addConstr(self.beta_varsX1[betaVarX1Index] <= self.clusterBits[clusterBitIndex][bitIndex],
                                        name=f"BetaX1_{betaVarX1Index}_BitPlus_{bitPlus}")

            for bitMinus in betaVarsBitsX1[betaVarX1Index][1]:
                parts = bitMinus.split('_')
                clusterBitIndex = int(parts[0][1:]); bitIndex = int(parts[1])
                self.model.addConstr(self.beta_varsX1[betaVarX1Index] <= 1 - self.clusterBits[clusterBitIndex][bitIndex],
        ...
        self.constrs = self.model.addConstrs(( gp.quicksum(self.clusterBits[int(bitPlus.split('_')[0][1:])][int(bitPlus.split('_')[1])]
                                            ...
                                            1 - (len(betaVarsBitsX1[indexBetaVarX1][0]) + len(betaVarsBitsX1[indexBetaVarX1][1]))
                                        <= self.beta_varsX1[indexBetaVarX1] for indexBetaVarX1 in range(len(self.beta_varsX1))),
                                        name="betaX1_Force1")
        ...
        for indexParametric in range(amountNonTrivialRestrictions):
            self.model.addConstr(self.bitsParametric[indexParametric] >= 0,
                                        name=f"ParametricPositive{indexParametric}")
            self.model.addConstr(self.bitsParametric[indexParametric] <= 1,
            ...
            for bitPlus in parametric_column[indexParametric][0]:
                if len(bitPlus) == 2: # b0
                    self.model.addConstr(self.bitsParametric[indexParametric] <= self.bit0[0],
                                        name=f"Parametric_{indexParametric}BitPlus{bitPlus}")
                elif bitPlus[4] == '0': # beta 0
                    self.model.addConstr(self.bitsParametric[indexParametric] <= self.beta_varsX0[int(bitPlus.split('_')[-1])],
                                        name=f"Parametric_{indexParametric}BitPlus{bitPlus}")
                else: # beta 1
                    self.model.addConstr(self.bitsParametric[indexParametric] <= self.beta_varsX1[int(bitPlus.split('_')[-1])],
                                        name=f"Parametric_{indexParametric}BitPlus{bitPlus}")

            for bitMinus in parametric_column[indexParametric][1]:
                if len(bitMinus) == 2: # b0
                    self.model.addConstr(self.bitsParametric[indexParametric] <= 1 - self.bit0[0],
                                        name=f"Parametric_{indexParametric}bitMinus{bitMinus}")
                elif bitMinus[4] == '0': # beta 0
                    self.model.addConstr(self.bitsParametric[indexParametric] <= 1 - self.beta_varsX0[int(bitMinus.split('_')[-1])],
                                        name=f"Parametric_{indexParametric}bitMinus{bitMinus}")
                else: # beta 1
                    self.model.addConstr(self.bitsParametric[indexParametric] <= 1 - self.beta_varsX1[int(bitMinus.split('_')[-1])],
                                        name=f"Parametric_{indexParametric}bitMinus{bitMinus}")
                
        # 1 - N + sum(b+) + sum(1 - b-) <= beta
        self.constrs = self.model.addConstrs(( gp.quicksum(
                                                self.bit0[0] * (len(bitPlus) == 2)
                                                + self.beta_varsX0[int((bitPlus + "00000")[6:]) // 100_000 if len(bitPlus) > 2 else 0] * ((bitPlus + "22222")[4] == '0')
                                                + self.beta_varsX1[int((bitPlus + "00000")[6:]) // 100_000 if len(bitPlus) > 2 else 0] * ((bitPlus + "22222")[4] == '1')

                                                for bitPlus in parametric_column[indexParametric][0]) +
                                            gp.quicksum(
                                                1 
                                                - self.bit0[0] * (len(bitMinus) == 2)
                                                - self.beta_varsX0[int((bitMinus + "00000")[6:]) // 100_000 if len(bitMinus) > 2 else 0] * ((bitMinus + "22222")[4] == '0')
                                                - self.beta_varsX1[int((bitMinus + "00000")[6:]) // 100_000 if len(bitMinus) > 2 else 0] * ((bitMinus + "22222")[4] == '1')

                                                for bitMinus in parametric_column[indexParametric][1]) +
                                                
                                            1 - (len(parametric_column[indexParametric][0]) + len(parametric_column[indexParametric][1]))
                                        <= self.bitsParametric[indexParametric] for indexParametric in range(len(self.bitsParametric))), name="ParametricForce1Condition")        
...
class ScalarProblem:
    ...
    def _generate_patterns(self):
        ...
        self.subproblem.setup(amountBitsPerCluster=self.amountBitsPerCluster,
        ...
        while True:
            ...
            for betaIndex in range(self.amountBetaVarsPerX):
                if self.interventionValue == 0:
                    objCoeff += self.betaVarsCost[betaIndex] * self.subproblem.beta_varsX0[betaIndex].X 
                else:
